...
from sqlalchemy import Integer, and_, delete, desc, func, inspect, or_, select, update
...
async def _check_alerts(session, reading: MeterReading) -> None:
    ...
    if reading.utility_type == "electricity":
        ...
        if reading.pressure_bottom_bar is not None and reading.pressure_top_bar is not None:
            ...
            bottom_pressure_min = _rule_max(
                water_top_rule,
                settings.water_bottom_pressure_for_top_check_bar,
            )
            if (
                reading.pressure_bottom_bar > bottom_pressure_min
                and reading.pressure_top_bar < top_pressure_min
            ):
                _queue_alert(
                    alerts,
                    Alert(
        ...
        if reading.pressure_bar is not None and (reading.pressure_bar < gas_pressure_min or reading.pressure_bar > gas_pressure_max):
            _queue_alert(
                alerts,
                Alert(
                    device_id=reading.device_id,
                    ...
                    message=_rule_message(gas_pressure_rule, f"Gaz bosimi normadan tashqari: {reading.pressure_bar:.3f} bar"),
                ),
                gas_pressure_rule,
            )
