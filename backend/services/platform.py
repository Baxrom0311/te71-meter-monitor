...
async def ota_upload(
    ...
    filename = f"{firmware_mode}_{hardware_version or 'any'}_{version}.bin".replace("/", "_")
    ...
    async with SessionLocal() as session:
        await session.execute(
            update(Firmware)
            .where(and_(Firmware.firmware_mode == firmware_mode, Firmware.hardware_version == hardware_version))
            .values(active=False)
        )
...
async def ota_list() -> dict:
    async with SessionLocal() as session:
        rows = (await session.scalars(select(Firmware).order_by(desc(Firmware.uploaded)))).all()
    return {"firmware": [_as_dict(row) for row in rows]}
...
async def ota_check(device_id: str, current_version: str) -> dict:
    ...
    return {
        ...
        "url": f"/api/ota/firmware/{firmware.filename}",
        "size": firmware.size,
